...
def build_collection_router(
    ...
    async def get_collections(request: Request) -> list[CollectionView]:
        ...
        return [_collection_view(row) for row in await list_collections(sessions, user.id)]
    ...
    async def get_collection_files(
        ...
        user = await authenticate_request(request, sessions, settings)
        collection = await _require_collection(sessions, collection_id, user.id)
        collection_owner_id = collection.owner_id
        if file_index is not None:
            async with sessions() as session:
                pending_ingestion = await session.scalar(
                    select(AssetIngestionRow.id).where(
                        ...
                        AssetIngestionRow.status == "indexing",
                    )
                )
            if pending_ingestion is not None:
                await file_index.reconcile_collection(collection_owner_id, collection_id)
        async with sessions() as session:
            total = int(
            ...
            ingestions = (
                list(
                    await session.scalars(
                        ...
                        .order_by(AssetIngestionRow.asset_id, AssetIngestionRow.version.desc())
                    )
                )
                if asset_ids
                else []
        ...
        ingestion_by_asset: dict[str, AssetIngestionRow] = {}
        for item in ingestions:
            ingestion_by_asset.setdefault(item.asset_id, item)
...
def _collection_view(row: FileCollectionRow) -> CollectionView:
